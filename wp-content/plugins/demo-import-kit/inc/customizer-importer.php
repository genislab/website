<?php
/**
 * Class for the customizer importer used in the Demo Import Kit plugin.
 *
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new WP_Error(
				...
				sprintf(
					esc_html__( 'The customizer import file is missing! File path: %s', 'demo-import-kit' ),
					$import_file_path
				)
			);
		}
		// Get the upload data.
		$raw  = Demo_Import_Kit_Base_Class::data_from_file( $import_file_path );
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error(
				...
				esc_html__( 'No correct file format. Please make sure to use the correct customizer import file.', 'demo-import-kit' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'demo-import-kit' )
			);
		}

		// Import images.
		if ( apply_filters( 'dik/customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			require DIK_PATH . 'inc/customizer-option.php';

			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new DIK_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
